...
class PatternsTestCase(unittest.TestCase):
    ...
    def testAddNode(self):
        """Test for adding one node to the tree."""

        self.firstNode = Patterns("1")
        ...
        # Make sure that the single child node exists
        assert self.firstNode.name == "1"
        assert self.firstNode.count == 1
        assert len(self.firstNode.next) == 0
        
    def testAddTwoNodes(self):
        """Test for adding two nodes to the tree."""
